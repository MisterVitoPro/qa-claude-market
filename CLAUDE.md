...
|--------|---------|-------------|
| `qa-swarm` | 1.5.0 | Dual-client Claude Code and Codex quality pipeline with portable bundled-role loading: parallel specialist review, evidence-backed P0-P3 findings, a generated implementation spec and test plan, and a fresh-context test-first implementation handoff. Sourced externally from [github.com/MisterVitoPro/qa-swarm](https://github.com/MisterVitoPro/qa-swarm). |
...
| `plan-runner` | 1.14.0 | Dual-client Claude Code and Codex plan executor with portable bundled-role loading: file-disjoint parallel waves, TDD gates with one shared suite run per wave, configurable independent verification pipelined off the critical path (each wave commits first; its verifier reads a snapshot of that commit while the next wave runs), phased execution with run-state resume for large plans, honest token/coverage accounting, bug-driven fix-plan re-runs, optional Code Atlas sync, and structured pull requests. Codex uses native subagents; Claude Code can additionally opt into Agent Teams. Sourced externally from [github.com/MisterVitoPro/plan-runner](https://github.com/MisterVitoPro/plan-runner). |
...
| `ideas` | 0.7.0 | Claude Code and Codex idea-to-spec pipeline. The interview skill pins an existing-system baseline, runs scope-sized batched question waves with a category-coverage floor, records a resumable decided/assumed/open ledger, and gates the EARS-based spec with a binding auditor plus a biggest-miss critic. The plan skill emits a contracts-only plan for plan-runner or inline/subagent execution; the tickets skill projects ready tasks to GitHub. Both manifests, Codex-safe skill frontmatter, host-neutral question handling, and portable reviewer loading ship from [github.com/MisterVitoPro/ideas](https://github.com/MisterVitoPro/ideas). |
